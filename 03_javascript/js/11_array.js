 ...
 * 반복문
 *  - for
 *  - for in 문 : 각 속성명(index)을 매턴에 반환 객체순회에 적합
 *  - for of 문 : 각 속성값을 매턴에 반환
 */
function test2() {
    const arr = ['a', 'b', 'c', 'd', 'e'];
...
/**
 * 유사배열 
 * - Array 객체를 상속하지 않은 배열 모양만 흉내낸 객체 (index, length 속성만 있음)
 * - Array 객체가 제공하는 다른 메소드 사용불가
...
function test6(params) {
    ...
    // const buttons = [...document.querySelectorAll('button')];    
    // const reduce = buttons.reduce(function (arr , n) {
    //     arr.push(n);        
    //     return arr;
    // }, [])
    // console.log(reduce);
